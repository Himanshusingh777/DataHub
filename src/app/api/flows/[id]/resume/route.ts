...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  await db.prepare("UPDATE flows SET status = 'active' WHERE id = ?").run(id);
  await writeAudit({ workspaceId: flow.workspace_id ?? "default", userId, action: "flow.resumed", resource: id });
