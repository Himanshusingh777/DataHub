...
export async function GET(req: NextRequest) {
  try {
    ...
    return NextResponse.json({ ok: true, workspaces: await listWorkspaces(userId) });
  } catch (e) {
...
export async function POST(req: NextRequest) {
  ...
  try {
    const userId = await getUserId(req);
    const ws = await createWorkspace(userId, body.name.trim(), body.plan ?? "free");
    await writeAudit({ workspaceId: ws.id, userId, action: "workspace.created", resource: ws.id, meta: { name: ws.name } });
...
export async function DELETE(req: NextRequest) {
  ...
  try {
    ...
    if (ok) await writeAudit({ workspaceId: id, userId, action: "workspace.deleted", resource: id });
