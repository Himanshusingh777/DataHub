...
export async function GET(req: NextRequest) {
  ...
  const recentRuns = db.prepare(`
    SELECT r.id, r.flow_id AS flowId, COALESCE(f.source_name, f.source_id) AS flowName,
           ...
           datetime(r.started_at / 1000, 'unixepoch') AS startedAt
    FROM runs r
  ...
  if (whCred) {
    try {
      ...
      const tableAgg = db.prepare("SELECT COUNT(*) AS cnt, SUM(row_count) AS rows FROM catalog_tables WHERE user_id = ?").get(userId) as { cnt: number; rows: number | null };
      warehouse = {
        configured: true,
  ...
  const credRows = db.prepare("SELECT service FROM credentials WHERE user_id = ?").all(userId) as { service: string }[];
