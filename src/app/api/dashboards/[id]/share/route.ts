...
export async function POST(
  ...
  if (action === "revoke") {
    await db.prepare("UPDATE dashboards SET share_token = NULL, is_published = 0, updated_at = ? WHERE id = ?")
      .run(Date.now(), id);
  ...
  await db.prepare("UPDATE dashboards SET share_token = ?, is_published = 1, updated_at = ? WHERE id = ?")
