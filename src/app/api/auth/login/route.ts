...
export async function POST(req: NextRequest) {
  ...
  const rl = await checkRateLimit(rateLimitKey(requestIdentity(req), "auth.login"), RATE_LIMITS.auth);
  if (!rl.allowed) {
  ...
  try {
    const db = getDb();
    const user = await db.prepare("SELECT id, email, name, pass_hash FROM users WHERE email = ?").get(email) as
      | { id: string; email: string; name: string | null; pass_hash: string } | undefined;
    if (!user || !verifyPassword(body.password ?? "", user.pass_hash)) {
      await writeAudit({ action: "auth.login_failed", resource: email || "(empty)", ip: clientIp(req) });
    ...
    await writeAudit({ userId: user.id, action: "auth.login", resource: user.id, ip: clientIp(req) });
